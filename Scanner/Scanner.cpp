...
bool Scanner::scanForAddressesAndPatch()
{
	...
	if (!Patcher::patchIfNeeded("UnrestrictIdConsole1", ((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "unlockConsole_1Sig", unlockConsole_1Sig) + 6), UnrestrictIdConsoleNewInstructionVec)) {
		logErr("scanForAddressesAndPatch failed for unlockConsole_1Sig");
		return false;
	}

	const char unlockConsole_2Sig[] = "BA ? ? ? ? 49 8B CF FF 50 18";
	if (!Patcher::patchIfNeeded("UnrestrictIdConsole2", ((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "unlockConsole_2Sig", unlockConsole_2Sig) + 1), UnrestrictIdConsoleNewInstructionVec)) {
	...
	if (!idGameLocalManager::acquireIdGameLocalPtr(MemHelper::FindPtrFromRelativeOffset(((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "GetidGameLocalPtr", idGameLocalPtrSig)), 3, 7))) {
	...
	if (!idEngineLocalManager::acquireidEngineLocalPtr(MemHelper::FindPtrFromRelativeOffset(((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "GetidEngineLocal", idEngineLocalSig)), 3, 7))){
	...
	if (!idCvarManager::acquireFindInternalFuncPtr((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "findCvarInternalFuncSig", findCvarInternalFuncSig))) {
	...
	if (!idCvarManager::acquireSetInternalFuncAddr((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "setCvarInternalFuncSig", setCvarInternalFuncSig))) {
	...
	if (!idCmdManager::acquireIdCmdExecuteFuncAddr((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "executeCmdFuncSig", executeCmdFuncSig))) {
	...
	if (!TypeInfoManager::acquirreFindClassInfoFuncPtr((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "FindClassInfoFuncPtrSig", classInfoFuncSig))) {
		logErr("scanForAddressesAndPatch failed for classInfoFuncSig");
		return false;
	}

	const char enumInfoFuncSig[] = "48 89 5C 24 ? 56 0F B6 02";
	if (!TypeInfoManager::acquirreFindEnumInfoFuncPtr((uintptr_t)MemHelper::ModulePatternScan(Config::getModuleName(), "FindEnumInfoFuncPtrSig", enumInfoFuncSig))) {
