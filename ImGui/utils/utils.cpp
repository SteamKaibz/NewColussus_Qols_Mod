...
static RenderingBackend_t g_eRenderingBackend = NOBackEnd;
...
namespace Utils {
	...
	HWND GetProcessWindow( ) {
		...
		logInfo("GetProcessWindow calledK...");
		HWND hwnd = nullptr;
		EnumWindows(::EnumWindowsCallback, reinterpret_cast<LPARAM>(&hwnd));

		while (!hwnd) {
			EnumWindows(::EnumWindowsCallback, reinterpret_cast<LPARAM>(&hwnd));
			logInfo("[!] Waiting for window to appear.\n");
			std::this_thread::sleep_for(std::chrono::milliseconds(200));
		}

		char name[128];
		GetWindowTextA(hwnd, name, RTL_NUMBER_OF(name));
		logInfo("Got window with name: '%s'\n", name);
